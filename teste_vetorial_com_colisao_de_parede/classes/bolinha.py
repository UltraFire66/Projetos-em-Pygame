...
class Bolinha:
    ...
    def update(self):
        ...
        #fatorDistanciaObstaculo = ((self.raioDeteccao - distanciaBordaObs)/self.raioDeteccao)*1.2 #porcentagem que indica o quão próximo do obstaculo a bolinha está (100% quando encostar)
        ...
        if(distanciaParede.length() < self.raioDeteccao):
            resultante = ((self.vel.length()*10)*self.direcao) + (self.vel.length()*10)*direcaoParede*fatorDistanciaParede
            direcaoResultante = vector.normalize(resultante)
        else:
            direcaoResultante = self.direcao
